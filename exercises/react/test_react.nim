...
suite "React":
  ...
  test "callbacks should only be called once even if multiple dependencies have changed":
    ...
    var changed4 = 0
    discard c1.addCallback(proc (val: int) = echo fmt"c1 callback: {val}")
    discard c2.addCallback(proc (val: int) = echo fmt"c2 callback: {val}")
    discard c3.addCallback(proc (val: int) = echo fmt"c3 callback: {val}")
    discard c4.addCallback(proc (val: int) = echo fmt"c4 callback: {val}")
  ...
  test "callbacks should not be called if dependencies change in a way that the final value of the compute cell does not change":
    ...
    var changed = 0
    discard plusOne.addCallback(proc (val: int) = echo fmt"plusOne callback: {val}")
    discard minusOne.addCallback(proc (val: int) = echo fmt"minusOne callback: {val}")
    discard alwaysTwo.addCallback(proc (val: int) = echo fmt"alwaysTwo callback: {val}")
